...
/*   Updated: 2020/03/05 00:05:31 by bpace            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*delete(char *sequence, int n) {
	...
	char *ans = malloc(sizeof(char) * strlen(sequence));
	ans[strlen(sequence) - 1] = '\0';
...
char	*fph(char *sequence, int nd, int start, int end) {
	...
	if (sequence[start + 1] == sequence[end - 1] && nd > 0) {
		ans = delete(sequence, start);
		ans = delete(ans, end - 1);
		ans = fph(ans, nd - 2, start, end - 2);
		if (ans)
			return (ans);
	}
	...
	if (sequence[start] == sequence[end - 1] && nd > 0) {
		ans = fph(delete(sequence, end), nd - 1, start, end - 1);
		if (ans)
			return (ans);
	}
	if (nd > 0) {
